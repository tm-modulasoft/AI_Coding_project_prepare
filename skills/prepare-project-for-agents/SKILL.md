...
Do not invent a stack or style the code does not use. Do not commit unless asked.
...
Before writing files, read `references/instruction-prompt.md` and `references/writing-rules.md`.
...
3. Draft the file tree. Skip helpers with no evidence (no UI → no theming file).
4. Write on-demand helpers first (`docs/agents/` unless the host already uses `.agents/` or `.claude/references/`), then compress into root `AGENTS.md`, then thin shims.
5. Prune every line that would not cause a mistake if removed.
6. Run the quality checklist in `references/writing-rules.md`. Smoke-check that listed commands exist in scripts/CI.
7. Report using `templates/report.md`. Do not commit.
